...
import com.deagenstroup.agendaassistant.logic.DailyPlannerUserInterface;
...
//                Log.d("TESTING", "Current Task: " + task.getName() + " time: " + task.getTimeRemainingString());
    ...
    /**
     * Registers an Alarm with the AlarmManager of the system which executes fivcheck to see if the service is
     * still running and restart it if it is not.
     */
    public void setServiceWakeupAlarm() {
        ScheduledToDoTask task = taskManager.getCurrentTask();
        if(task == null)
            return;
        Duration timeRemaining = task.getTimeRemaining();
        if(timeRemaining == null)
            return;

        AlarmManager alarmManager = (AlarmManager) getSystemService(Context.ALARM_SERVICE);
        Intent receiverIntent = new Intent(this, TaskAlarmReceiver.class);
        PendingIntent alarmIntent = PendingIntent.getBroadcast(this, 0, receiverIntent, 0);
        alarmManager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP,
                SystemClock.elapsedRealtime() + timeRemaining.toMillis() - 5000, alarmIntent);
//        alarmManager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP,
//                SystemClock.elapsedRealtime() + 15000, alarmIntent);
    }
